...
class WaveFunctionCollapse {
	private static final int PROPAGATION_RADIUS = 3;  // TODO: Distance that the wave propagates before collapsing.
    ...
    private void propagateWave() { 
        ...
        for (int row = 0; row < TileMapRenderer.GRID_HEIGHT; row++) {
            for (int col = 0; col < TileMapRenderer.GRID_WIDTH; col++) {
    ...
    private void reduceEntropy(Cell cell, int row, int col) {
        ...
        if (row < TileMapRenderer.GRID_HEIGHT - 1) {
        ...
        if (col < TileMapRenderer.GRID_WIDTH - 1) {
